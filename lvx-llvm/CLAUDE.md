...
When adding a pattern, check `LVXInstrPatterns.td` first — a few hand-written ones are now redundant with generated ones (the `BinOpImm` instantiations for `add`/`and`/`or`/`xor` in particular) and should not be extended. Instruction *flags* (`isCall`, `isReturn`, `isBranch`, implicit `$ra`) cannot be added there — TableGen cannot set a field on an existing def — and live in `lvx-mds/lvx-family/BE/LLVM/instr-flags`, keyed by generated def name.
