...
Sibling components (all submodules of `lvx-csw`): `lvx-newlib` (libc), `lvx-gdb` (debugger), `lvx-llvm/llvm-project` and `lvx-mlir/llvm-project` (two branches of one LLVM fork), and `lvx-gem5` — the gem5-based ISS. Its `arch/lvx` SE-mode port runs scalar integer **and** floating-point programs; the full scalar f16/f32/f64 FP surface is implemented in the runtime shim over Berkeley SoftFloat (RISC-V-conformant — see `lvx-gem5/STATUS.md` and the `lvx-fp-matches-riscv` project memory).
...
**`BE/LLVM` generates most of the LLVM back end's target description** and is installed. Six files in `lvx-llvm/llvm-project/llvm/lib/Target/LVX/` are its output: `LVXInstrEncodings.td` (every instruction format class and instruction def, with full encodings — including the `.W`/`.M`/`.X`/`.Y` immediate-extension formats), `LVXInstrFormats.td` (every operand type), `LVXModifiers.h`, `LVXImmediateExtensions.inc`, `LVXInstrPatterns.td` (87 SelectionDAG patterns read out of each opcode's `Behavior`) and `LVXMemoryTable.inc` (the load/store instructions by access width and extension, plus the `variant` values). **Do not hand-edit them.**

What stays hand-written there is `LVXInstrInfo.td` — SelectionDAG nodes, pseudos, and the patterns the description has nothing to read for (integer min/max, shifts, `select`, branches, compares, `fneg`/`fabs`/`fcopysign`, bitconverts) — plus the instruction flags MDS cannot state (`isCall`/`isReturn`/…), which live in `lvx-mds/lvx-family/BE/LLVM/instr-flags`. See `lvx-llvm/CLAUDE.md` and the `BE/LLVM` section of `lvx-mds/CLAUDE.md`.

**The min/max NaN split is a toolchain-wide invariant.** LVX has both IEEE families and the assembly mnemonics are the trap: `fminn`/`fmaxn` (helpers `f*_minNum`/`f*_maxNum`) are 754-2008 minNum, returning the *numeric* operand, and are what C's `fmin`/`fmax` and LLVM's `fminnum`/`fmaxnum` require; `fmin`/`fmax` (helpers `f*_min`/`f*_max`) are 754-2019 minimum, *propagating* the NaN, matching `fminimum`/`fmaximum`. Every non-NaN input gives the same answer from both, so a wrong pairing survives ordinary testing — lvx-gcc emitted `fmind` for C's `fmin` until 2026-08-04. Pinned on the ISS by `validation/tests/ir/minmax-nan.ll`.
