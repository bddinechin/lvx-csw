...
Everything LVX-specific lives under `mlir/{include,lib,test}/**/LVX*` and `mlir/{include,lib,test}/Conversion/ConvertToLVX`; the rest of the tree is unmodified upstream LLVM/MLIR.

### Ground truth for the LVX ISA (outside this repo)

- `/home/guembu/bd3/lvx-csw/lvx-mds/refs/**` is the **authoritative** machine-description database for the LVX ISA (only the `lvx_v1` core). Treat everything else as derived from it.
- `/home/guembu/bd3/lvx-csw/lvx-target/` extracts/prunes that database via `make all` (`prune_opcode.pl`, `prune_format.pl`) into `lvx_Opcode.txt`, `lvx_Format.yml`, `lvx_Modifier.yml`, `lvx_Convention.yml`, `lvx_Synthetic.yml` — these are what the dialect's op set, modifier enums, and ABI (`lvx_Convention.yml`'s `regular` convention: 12 argument registers `$r0-$r11`, 4 result registers `$r0-$r3`, `$ra` return address) are derived from.
- If the dialect needs data not already exported there (e.g. `Scheduling`/`Resource`/`Bundle` tables for a future software-pipelining pass, or `RegFile`/`RegClass` tables for SIMD register pairs/quads), add a new `prune_*.pl` script to `lvx-target` reading from `lvx-mds/refs/**/lvx_v1/`, following the existing scripts' pattern — do not hand-derive ISA facts from other sources.
- `/home/guembu/bd3/LLVM/llvm-project/llvm/lib/Target/LVX/` is a **separate, independent** SelectionDAG LLVM backend for LVX (own git repo, branch `lvx-llvm`). It's a useful cross-reference for mnemonic spelling and register layout but is **not** the source of truth — it was hand-written in parallel and may drift from the ground truth above.

## Build

Configure once (this checkout only needs the `mlir` project; `LLVM_TARGETS_TO_BUILD=X86` is just a placeholder native target, unrelated to the LVX work):
...
Dialect scaffolding, register allocation, and real assembly emission are built and verified end to end — real kernels compiled through the whole pipeline, assembled/linked with the real `lvx-mbr-as`/`lvx-mbr-ld`, and executed on real `lvx-gem5`. Register allocation is a 3-step linear scan (`docs/lvx/RegisterAllocation.md`; Step 1 live intervals, Step 2 assignment-only with a hard error on pressure, Step 3 the same scan plus real spilling), with explicit coalescing for loop-carried `iter_args`, branch arguments, and tied-operand instructions (e.g. `ffma`/`ffms`, whose accumulator operand shares a register with its own result on real hardware). Hardware loops (`LOOPDO`) and a handful of real-hardware-only instruction quirks (`divmod`'s register-pair destination, `$ra` save/restore, call-site ABI register pinning) are also done — see `docs/lvx/HardwareLoops.md`, `docs/lvx/AssemblyEmission.md`, and `docs/lvx/EndToEndValidation.md` for the full design and validation history.

Still not built: software pipelining, and SIMD (`!lvx.pair`/`!lvx.quad` for 128-/256-bit values, lane/blend/guard ops) — both later phases per the original plan, needing new `lvx-target` extraction scripts for `Scheduling`/`Resource`/`Bundle` and `RegFile`/`RegClass` (`PGR`/`QGR`) data respectively.
