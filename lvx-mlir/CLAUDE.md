...
from a standalone sibling repo; paths below that start with `mlir/`, `llvm/`, `build/` or
`docs/` are relative to `llvm-project/`, so `cd llvm-project` first). Everything LVX-specific
...
unmodified upstream LLVM/MLIR. Design/validation docs are in `llvm-project/docs/lvx/`.
